...
- `basic_auth *`: Adds basic authentication to all paths (`*`). You can limit authentication to a specific path by replacing the `*` with something like `/private/*`.

If you want to protect the entire file server with a username and password, include the `basic_auth` block using the hashed password you generated earlier and replace `your-username` with you actual username.
